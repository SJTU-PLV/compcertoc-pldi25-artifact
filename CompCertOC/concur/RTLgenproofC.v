...
Let ge : CminorSel.genv := Genv.globalenv se prog.
Let tge : RTL.genv := Genv.globalenv se tprog.

(** Relationship between the global environments for the original
  CminorSel program and the generated RTL program. *)

Lemma functions_translated:
  forall (v tv: val) (f: CminorSel.fundef),
  Genv.find_funct ge v = Some f ->
  Val.lessdef v tv ->
  exists tf,
  Genv.find_funct tge tv = Some tf /\ transl_fundef f = OK tf.
Proof.
  intros v tv f Hf Hv. apply (Genv.find_funct_transf_partial_id TRANSL).
...
Proof.
  ...
  exists rs'; exists tm, EXT; split. eauto.
  ...
  split. congruence.
  split; auto.
  constructor; eauto; try red; intros; eauto.
  (* general case *)
  split.
...
Proof.
  intros; red; intros. inv TE.
(* normal case *)
  exploit H0; eauto. intros [rs1 [tm1 [EXT1 [EX1 [ME1 [RR1 [RO1 ACI]]]]]]].
  edestruct eval_operation_lessdef as [v' [ ]]; eauto.
  exists (rs1#rd <- v'); exists tm1, EXT1.
(* Exec *)
...
Proof.
  intros; red; intros. inv TE.
  exploit H0; eauto. intros [rs1 [tm1 [EXT1 [EX1 [ME1 [RES1 [OTHER1 ACI]]]]]]].
...
Proof.
  intros; red; intros; inv TE.
  exploit H0; eauto. intros [rs1 [tm1 [EXT1 [EX1 [ME1 [OTHER1 ACI1]]]]]].
  assert (tr_expr f.(fn_code) map pr (if va then ifso else ifnot) (if va then ntrue else nfalse) nd rd dst).
    destruct va; auto.
  exploit H2; eauto. intros [rs2 [tm2 [EXT2 [EX2 [ME2 [RES2 [OTHER2 ACI2]]]]]]].
  exists rs2; exists tm2,EXT2.
(* Exec *)
...
Proof.
  ...
  exists rs1; exists tm. exists EXT.
(* Exec *)
  split. eexact EX1.
(* Match-env *)
  split.
...
Proof.
  intros; red; intros. inv TE.
  exploit H0; eauto. intros [rs1 [tm1 [EXT1 [EX1 [ME1 [RR1 [RO1 ACI1]]]]]]].
  exploit external_call_mem_extends; eauto.
  intros [v' [tm2 [A [B [C [D E]]]]]].
  exists (rs1#rd <- v'); exists tm2. exists C.
(* Exec *)
  split. eapply star_right. eexact EX1.
...
Proof.
  intros; red; intros. inv TE.
  exploit H3; eauto. intros [rs1 [tm1 [EXT1 [EX1 [ME1 [RR1 [RO1 ACI1]]]]]]].
  exploit external_call_mem_extends; eauto.
  intros [v' [tm2 [A [B [C [D E]]]]]].
  rewrite <- Genv.find_funct_find_funct_ptr in H0.
  exploit functions_translated; eauto. simpl. intros [tf [P Q]].
  inv Q.
  exists (rs1#rd <- v'); exists tm2, C.
(* Exec *)
...
Proof.
  ...
  assert (tr_condition (fn_code f) map pr (if va then b else c) (if va then n2 else n3) ntrue nfalse).
    destruct va; auto.
  exploit H2; eauto. intros [rs2 [tm2 [EXT2 [EX2 [ME2 [OTHER2 ACI2]]]]]].
  exists rs2; exists tm2, EXT2.
(* Exec *)
...
Proof.
  ...
  assert (map_wf (add_letvar map r)).
    eapply add_letvar_wf; eauto.
  exploit H2; eauto. eapply match_env_bind_letvar; eauto.
  intros [rs2 [tm2 [EXT2 [EX2 [ME3 [OTHER2 ACI2]]]]]].
  exists rs2; exists tm2, EXT2.
(* Exec *)
...
Definition transl_exitexpr_prop
  ...
  forall tm cs f map ns nexits rs
    ...
    (ME: match_env map e le rs)
    (EXT: Mem.extends m tm),
  exists nd, exists rs', exists tm', exists Hm',
     star step tge (State cs f sp ns rs tm) E0 (State cs f sp nd rs' tm')
...
- (* XEjumptable *)
  ...
  exploit transl_expr_correct; eauto. intros (rs1 & tm1 & EXT1 & EXEC1 & ME1 & RES1 & PRES1 & ACI1).
  exists nd, rs1, tm1, EXT1.
  split. eapply star_right. eexact EXEC1. eapply exec_Ijumptable; eauto. inv RES1; auto. traceEq.
  intuition auto. eauto.
- (* XEcondition *)
  exploit transl_condexpr_correct; eauto. intros (rs1 & tm1 & EXT1 & EXEC1 & ME1 & RES1 & ACI1).
  exploit IHeval_exitexpr; eauto.
  instantiate (2 := if va then n2 else n3). destruct va; eauto.
  intros (nd & rs2 & tm2 & EXT2 & EXEC2 & EXIT2 & ME2 & ACI2).
  exists nd, rs2, tm2, EXT2.
  split. eapply star_trans. apply plus_star. eexact EXEC1. eexact EXEC2. traceEq.
  intuition auto. etransitivity; eauto.
- (* XElet *)
  exploit transl_expr_correct; eauto. intros (rs1 & tm1 & EXT1 & EXEC1 & ME1 & RES1 & PRES1 & ACI1).
  assert (map_wf (add_letvar map r)).
    eapply add_letvar_wf; eauto.
...
Inductive match_states: ext_world -> CminorSel.state -> RTL.state -> Prop :=
  | match_state:
      forall f s k sp e m tm cs tf ns rs map ncont nexits ngoto nret rret wp Hm
        (MWF: map_wf map)
  ...
  | match_callstate:
      forall vf args targs k m tm cs tvf wp Hm
...
Theorem transl_step_correct:
  ...
  forall wp R1, match_states wp S1 R1 ->
  exists R2,
  (plus RTL.step tge R1 t R2 \/ (star RTL.step tge R1 t R2 /\ lt_state S2 S1))
  /\ match_states wp S2 R2.
Proof.
  induction 1; intros wp R1 MSTATE; inv MSTATE.
...
Lemma transl_initial_states:
  forall q1 q2 S, match_query (cc_c ext) w q1 q2 -> CminorSel.initial_state ge q1 S ->
  exists R, RTL.initial_state tge q2 R /\ match_states w S R.
Proof.
...
Lemma transl_final_states:
  forall wp S R r1, match_states wp S R -> CminorSel.final_state S r1 ->
...
Lemma transl_external_states:
  forall wp S R q1, match_states wp S R -> CminorSel.at_external ge S q1 ->
  exists wx q2, RTL.at_external tge R q2 /\ ext_acci wp (get wx) /\ GS.match_query (c_ext) wx q1 q2 /\ se = se /\
  forall r1 r2 S' wp'', (get wx) o-> wp'' -> GS.match_reply (c_ext) (CallconvBig.set wx wp'') r1 r2 -> CminorSel.after_external S r1 S' ->
  exists R', RTL.after_external R r2 R' /\ match_states wp'' S' R'.
Proof.
  intros. inv H0. inv H.
  exploit functions_translated; eauto. intros [tf' [P TF]].
  monadInv TF.
  exists (extw m tm Hm). eexists; intuition idtac.
  - econstructor; eauto.
...
Theorem transf_program_correct prog tprog:
  ...
  GS.forward_simulation (c_ext) (CminorSel.semantics prog) (RTL.semantics tprog).
Proof.
  intro MATCH. constructor.
  eapply GS.Forward_simulation.
  + try fsim_skel MATCH.
  + intros se1 se2 w Hse Hse1.
    eapply GS.forward_simulation_star_wf with (order := lt_state);
