...
Lemma ros_address_tailcall:
  forall tge ros ls1 ls2,
  ros_compatible_tailcall ros = true ->
  ros_address tge ros (return_regs ls1 ls2) = ros_address tge ros ls2.
Proof.
  unfold ros_compatible_tailcall, ros_address; intros.
...
Lemma add_equations_builtin_eval:
  forall ef env args args' e1 e2 m1 m1' rs ls ge sp vargs t vres m2 (Hm: Mem.extends m1 m1'),
  wt_regset env rs ->
  match ef with
...
Proof.
    ...
    /\ ext_acci (extw m1 m1' Hm) (extw m2 m2' Hm')).
  {
    ...
    intros (vres' & m2' & C & D & E & F & G).
    exists vargs', vres', m2', E. intuition auto.
  ...
  intros (vargs' & A).
  simpl in H3; inv H3.
  exists vargs', Vundef, m1', Hm. intuition auto. simpl. constructor. reflexivity.
Qed.
...
Lemma functions_translated:
  ...
  Val.lessdef v tv ->
  exists tf,
  Genv.find_funct tge tv = Some tf /\ transf_fundef f = OK tf.
Proof.
  intros v tv f Hf Hv.
...
Lemma ros_address_lessdef:
  ...
  Genv.find_funct ge (RTL.ros_address ge ros rs) = Some fd ->
  add_equation_ros ros ros' e = Some e' ->
  satisf rs ls e' ->
  Val.lessdef (RTL.ros_address ge ros rs) (LTL.ros_address tge ros' ls).
Proof.
  unfold RTL.ros_address, LTL.ros_address; intros.
...
Inductive match_states: ext_world -> RTL.state -> LTL.state -> Prop :=
  | match_states_intro:
      forall s f sp pc rs m ts tf ls m' an e env wp Hm
        (STACKS: match_stackframes s ts (fn_sig tf))
        (FUN: transf_function f = OK tf)
        (ANL: analyze f env (pair_codes f tf) = Some an)
        (EQ: transfer f env (pair_codes f tf) pc an!!pc = OK e)
  ...
  | match_states_return:
      ...
      match_states wp (RTL.Returnstate s res m)
...
Lemma step_simulation:
  ...
  forall wp S1', match_states wp S1 S1' ->
  exists S2', plus LTL.step tge S1' t S2' /\ match_states wp S2 S2'.
Proof.
  induction 1; intros WT wp S1' MS; inv MS; try UseShape.
...
- exploit exec_moves; eauto. intros [ls1 [X Y]].
  ...
  econstructor; eauto. instantiate (1:= Q). etransitivity; eauto. etransitivity; eauto.
...
- assert (SF: Archi.ptr64 = false) by (apply Archi.splitlong_ptr32; auto).
  ...
  assert (ACCI1: ext_acci (extw m m'0 Hm) (extw m1 m1' EXT1)).
  eapply ext_acci_storev; eauto.
  assert (ACCI2: ext_acci (extw m1 m1' EXT1) (extw m' m2' EXT2)).
  eapply ext_acci_storev; eauto. eapply Val.add_lessdef; eauto.
  econstructor; split.
  eapply plus_left. econstructor; eauto.
  eapply star_trans. eexact X.
  eapply star_left.
...
- set (sg := RTL.funsig fd) in *.
  ...
  apply return_regs_agree_callee_save. instantiate (1:= Q).
...
- exploit (exec_moves mv1); eauto. intros [ls1 [A1 B1]].
  ...
  intros (C & vargs' & vres' & m'' & Hm'' & D & E & F & G).
  ...
  exploit satisf_successors; eauto. simpl; eauto.
  intros [enext [U V]].
  econstructor; eauto. etransitivity; eauto. etransitivity; eauto.
...
- destruct (transf_function_inv _ _ FUN).
  ...
  exploit ext_acci_free. apply H0. rewrite H10.  apply P. intro ACCI.
  inv WTI; MonadInv.
+ (* without an argument *)
  exploit (exec_moves mv); eauto. intros [ls1 [A1 B1]].
  econstructor; split.
  ...
  instantiate (1:= Q). etransitivity; eauto. etransitivity; eauto.
  constructor.
+ (* with an argument *)
  exploit (exec_moves mv); eauto. intros [ls1 [A1 B1]].
  econstructor; split.
  ...
  apply return_regs_agree_callee_save. etransitivity; eauto. etransitivity; eauto.
...
- rewrite FIND in FIND0. inv FIND0.
  ...
  econstructor; eauto.
  eauto. eauto. traceEq.
  econstructor; eauto. instantiate (1:= V). etransitivity; eauto. etransitivity; eauto.

(* external function *)
- rewrite FIND in FIND0. inv FIND0.
  edestruct functions_translated as (tf & TFIND & FUN); eauto.
  erewrite <- sig_function_translated in STACKS; eauto.
  exploit external_call_mem_extends; eauto. intros [v' [m'' [F [G [J [K L]]]]]].
  simpl in FUN; inv FUN.
  assert (ACCI: ext_acci (extw m m'0 Hm) (extw m' m'' J)).
...
Lemma initial_states_simulation:
  forall q1 q2 st1,
    GS.match_query (c_ext @ cc_c_locset) (se, (w, base_sg)) q1 q2 ->
    RTL.initial_state ge q1 st1 ->
    Val.has_type_list (cq_args q1) (sig_args (cq_sg q1)) ->
  exists st2, LTL.initial_state tge q2 st2 /\ match_states (get w) st1 st2.
Proof.
  ...
  set (rs0 := initial_regs (fn_sig x) rs). destruct w eqn: Hw. inv H1.
...
Lemma final_states_simulation:
  forall wp st1 st2 r1, match_states wp st1 st2 -> RTL.final_state st1 r1 ->
  exists wp' r2,
    LTL.final_state st2 r2 /\ (get w) o-> wp' /\ ext_acci wp wp' /\
    GS.match_reply (c_ext @ cc_c_locset) (se, ((CallconvBig.set w wp'), base_sg)) r1 r2.
Proof.
  intros. inv H0. inv H. inv STACKS. cbn in * |- .
  exists (extw m m' Hm). eexists; split. econstructor; eauto. split. auto. split. auto.
  eexists. split. constructor; CKLR.uncklr; eauto. constructor.
  econstructor; eauto.
...
Lemma external_states_simulation:
  forall wp st1 st2 q1, match_states wp st1 st2 -> RTL.at_external ge st1 q1 ->
  exists (wx : GS.ccworld (c_ext @ cc_c_locset)) q2, LTL.at_external tge st2 q2 /\ ext_acci wp (fst (get wx)) /\ GS.match_query (c_ext @ cc_c_locset) wx q1 q2 /\
  forall r1 r2 st1' (wp'': ext_world), ext_acce (fst (get wx)) wp'' -> GS.match_reply (c_ext @ cc_c_locset) (local_set wx wp'') r1 r2 -> RTL.after_external st1 r1 st1' ->
    Val.has_type (cr_retval r1) (proj_sig_res (cq_sg q1)) ->
  exists st2', LTL.after_external tge st2 r2 st2' /\ match_states wp'' st1' st2'.
Proof.
  intros. inv H0. inv H.
  ...
  erewrite <- sig_function_translated in STACKS; eauto.
  simpl in FUN; inv FUN.
  exists (se, ((extw m m' Hm), sg)), (lq tvf sg ls m'). intuition idtac; cbn in *.
  - econstructor; eauto.
  - destruct LF; try discriminate.
    eexists; split; econstructor; CKLR.uncklr; eauto. constructor.
    destruct v; cbn in *; congruence.
  - destruct H0 as (ri & Hri1 & Hri2).
    inv H1. inv Hri1. inv Hri2. inv H. inv H5.
...
Theorem transf_program_correct prog tprog:
  ...
  GS.forward_simulation (wt_c @ c_ext @ cc_c_locset)
    (RTL.semantics prog) (LTL.semantics tprog).
Proof.
  intros MATCH.
  eapply source_invariant_fsim; eauto using rtl_wt, wt_prog.
  revert MATCH.
  set (ms := fun w se sg wp s s' => match_states prog tprog w se sg wp s s').
  ...
  + intros se1 se2 w Hse Hse1. simpl in w.
    ...
    Compute (GS.gworld (c_ext @ cc_c_locset)). *)
    (*set (ms' := ms w se sg).
    set (ms'' := fun (wp: (GS.gworld (c_ext @ cc_c_locset))) s1 s2 => ms' (fst wp) s1 s2). *) 
  eapply GS.forward_simulation_plus with (match_states :=  fun wp s1 s2 => ms (fst (snd w)) (fst w) (snd (snd w)) (fst wp) s1 s2);
    cbn -[wt_c] in *; subst;
  destruct w as [xse [w sg]]; inv Hse.
...
- intros gw s1 s2 r1 Hs (Hs1 & [xse xsg] & Hxse & Hr1 & WTr1). cbn in *. subst.
  edestruct final_states_simulation as (? & ? & ? & ? & ? & ?); eauto.
  exists x0. exists (x,tt). intuition auto. split; auto. split; auto.
- cbn. intros gw s1 s2 q1 Hs (Hq1 & [xse xsg] & [? ?] & ? & WTs1 & ? & WT). subst.
  edestruct external_states_simulation as ([se0 [wx sg1]] & q2 & Hq2 & ACI & Hq & Hr); eauto.
  simpl in *.
  eexists (_,( wx, sg1)), q2. cbn. repeat apply conj; eauto.
  intros r1 r2 s1' [gw'' x] [ACE1 ACE2] Hr12.
  intros (? & [? ?] & [? ?] & ? & ? & ? & ? & [? ?] & ? & ?).
  subst. inv Hq1. inv H1. cbn in *. assert (sg2 = sg0) by congruence. subst.
  eapply Hr; eauto.
- cbn. intros s1 t s1' (STEP & [xse xsg] & ? & Hs1 & Hs1') s2 Hs MS. subst.
