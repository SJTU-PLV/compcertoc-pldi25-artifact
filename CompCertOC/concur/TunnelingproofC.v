...
Variable lw: signature * ext_world.
Definition w := snd lw.
...
Inductive match_states: ext_world -> state -> state -> Prop :=
  | match_states_intro:
      forall s f sp pc ls m ts tls tm wp Hm
        (STK: list_forall2 match_stackframes s ts)
        (LS: locmap_lessdef ls tls)
        (ACI: ext_acci wp (extw m tm Hm))
        (ACE: ext_acce w (extw m tm Hm)),
      match_states wp (State s f sp pc ls m)
                   (State ts (tunnel_function f) sp (branch_target f pc) tls tm)
  | match_states_block:
    forall s f sp bb ls m ts tls tm wp Hm
        (STK: list_forall2 match_stackframes s ts)
        (LS: locmap_lessdef ls tls)
        (ACI: ext_acci wp (extw m tm Hm))
        (ACE: ext_acce w (extw m tm Hm)),
      match_states wp (Block s f sp bb ls m)
                   (Block ts (tunnel_function f) sp (tunneled_block f bb) tls tm)
  | match_states_interm_branch:
      forall s f sp pc bb ls m ts tls tm wp Hm
        (STK: list_forall2 match_stackframes s ts)
        (LS: locmap_lessdef ls tls)
        (ACI: ext_acci wp (extw m tm Hm))
        (ACE: ext_acce w (extw m tm Hm)),
      match_states wp (Block s f sp (Lbranch pc :: bb) ls m)
                   (State ts (tunnel_function f) sp (branch_target f pc) tls tm)
  | match_states_interm_cond:
      forall s f sp cond args pc1 pc2 bb ls m ts tls tm wp Hm
        (STK: list_forall2 match_stackframes s ts)
        (LS: locmap_lessdef ls tls)
        (ACI: ext_acci wp (extw m tm Hm))
        (ACE: ext_acce w (extw m tm Hm))
        (SAME: branch_target f pc1 = branch_target f pc2),
      match_states wp (Block s f sp (Lcond cond args pc1 pc2 :: bb) ls m)
                   (State ts (tunnel_function f) sp (branch_target f pc1) tls tm)
  | match_states_call:
      forall s vf ls m ts tvf tls tm wp Hm
        (STK: list_forall2 match_stackframes s ts)
        (LF: Val.lessdef vf tvf)
        (LS: locmap_lessdef ls tls)
        (ACI: ext_acci wp (extw m tm Hm))
        (ACE: ext_acce w (extw m tm Hm)),
      match_states wp (Callstate s vf ls m)
                   (Callstate ts tvf tls tm)
  | match_states_return:
      forall s ls m ts tls tm wp Hm
        (STK: list_forall2 match_stackframes s ts)
        (LS: locmap_lessdef ls tls)
        (ACI: ext_acci wp (extw m tm Hm))
        (ACE: ext_acce w (extw m tm Hm)),
      match_states wp (Returnstate s ls m)
...
Lemma tunnel_step_correct:
  ...
  forall wp st1' (MS: match_states wp st1 st1'),
  (exists st2', step tge st1' t st2' /\ match_states wp st2 st2')
  \/ (measure st2 < measure st1 /\ t = E0 /\ match_states wp st2 st1')%nat.
Proof.
...
- (* entering a block *)
  assert (DEFAULT: branch_target f pc = pc ->
    (exists st2' : state,
     ...
     /\ match_states wp (Block s f sp bb rs m) st2')).
...
- (* Lstore *)
  ...
  instantiate (1:= MEM'). etransitivity; eauto. etransitivity; eauto.
- (* Lcall *)
...
- (* Lbuiltin *)
  ...
  exploit external_call_mem_extends; eauto. intros (tvres & tm' & A & B & C & D & E).
...
Lemma transf_initial_states:
  forall q1 q2 st1, GS.match_query (locset_ext) lw q1 q2 -> initial_state ge q1 st1 ->
  exists st2, initial_state tge q2 st2 /\ match_states (get lw) st1 st2.
Proof.
  intros q1 q2 st1 Hq Hst1. destruct lw as [sg wb] eqn: Hlw. inv Hst1. inv Hq. CKLR.uncklr.
  ...
  - setoid_rewrite <- (sig_preserved (Internal f)).
    ...
    econstructor; eauto.
  - destruct wb eqn: Hwb. inv H7.
    econstructor; eauto.
    repeat constructor; eauto. simpl. reflexivity. unfold w. rewrite Hlw. simpl. reflexivity.
Qed.

Lemma transf_final_states:
  forall wp st1 st2 r1, match_states wp st1 st2 -> final_state st1 r1 ->
                   exists r2 wp', final_state st2 r2 /\ (get lw) o-> wp' /\ ext_acci wp wp'
                                                                 /\ GS.match_reply (locset_ext) (CallconvBig.set lw wp') r1 r2.
Proof.
  intros wp st1 st2 r1 Hst Hr1. inv Hr1. inv Hst. inv STK. inv H1.
  exists (lr tls tm).
  exists (extw m' tm Hm). split. constructor; auto. split. auto. split. auto.
...
Lemma transf_external_states:
  ...
  exists w q2, at_external tge st2 q2 /\ ext_acci wp (get w) /\ GS.match_query (locset_ext) w q1 q2 /\ se = se /\
  forall r1 r2 st1' wp'', (get w) o-> wp'' -> GS.match_reply (locset_ext) (CallconvBig.set w wp'') r1 r2 -> after_external ge st1 r1 st1' ->
  exists st2', after_external tge st2 r2 st2' /\ match_states wp'' st1' st2'.
Proof.
  intros. inv H0. inv H.
  exploit functions_translated; eauto. cbn. intros TFIND.
  exists (sg, (extw m tm Hm)), (lq tvf sg tls tm); intuition idtac.
  - econstructor; eauto.
  ...
  - inv H2. inv H0. inv H6.
    ...
    intro. apply ext_lessdef with (extw m tm Hm). apply result_regs_inject; auto.
    intro. apply ext_lessdef. auto. reflexivity. etransitivity. eauto. simpl in H.
...
Theorem transf_program_correct prog tprog:
  ...
  GS.forward_simulation (locset_ext) (LTL.semantics prog) (LTL.semantics tprog).
Proof.
  intros MATCH. constructor.
  eapply GS.Forward_simulation.
  + fsim_skel MATCH.
  + intros se1 se2 w Hse Hse1.
  eapply GS.forward_simulation_opt; destruct w as [sg w], Hse.
  ...
  - intros. exploit tunnel_step_correct; eauto. apply H.
    eauto.
